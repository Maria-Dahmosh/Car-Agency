...
const CarAgencyManager = {
  ...
  searchAgency(idOrName) {
    for (const agencyObj of this.agencies) {
      if (
        agencyObj["agencyName"] === idOrName ||
        agencyObj["agencyId"] === idOrName
      ) {
        return agencyObj;
      }
    }
  ...
  getAllAgencies() {
    const agencyNames = [];
    for (const agencyObj of this.agencies) {
      agencyNames.push(agencyObj.agencyName);
    }
    return agencyNames;
  },
  ...
  addCarToAgency(agencyId, car) {
    for (const agencyObj of this.agencies) {
      if (agencyObj.agencyId === agencyId) {
        agencyObj.cars.push(car);
        return true;
      }
    }
    return false;
  ...
  removeCarFromAgency(agencyId, carId) {
    const removefromagency = this.agencies.find(
      (agencyObj) => agencyObj.agencyId === agencyId
    );
    if (removefromagency) {
      const removedCarIndex = removefromagency.cars.findIndex(
        (car) => car.carNumber === carId
      );
      if (removedCarIndex !== -1) {
        removefromagency.cars.splice(removedCarIndex, 1);
        return true;
      }
    }
  ...
  changeAgencyCashOrCredit(agencyId, cashOrCredit) {
    const updateAgency = this.agencies.find(
      (agencyObj) => agencyObj.agencyId === agencyId
    );
    if (updateAgency) {
      if (updateAgency.cash >= cashOrCredit) {
        updateAgency.cash -= cashOrCredit;
      } else {
        updateAgency.cash = 0;
        updateAgency.credit -= cashOrCredit - updateAgency.cash;
      }
      return true;
    }
    return false;
  ...
  updateCarPrice(agencyId, carId, newPrice) {
    const updatedPriceObj = this.agencies.find(
      (agencyObj) => agencyObj.agencyId === agencyId
    );
    if (updatedPriceObj) {
      for (const car of updatedPriceObj.cars) {
        // Iterate through models of the car
        for (const model of car.models) {
          // Check if car number matches
  ...
  transferCarBetweenAgencies(fromAgencyId, toAgencyId, carId) {
    const fromAgency = this.agencies.find(
      (agencyObj) => agencyObj.agencyId === fromAgencyId
    );
    const toAgency = this.agencies.find(
      (agencyObj) => agencyObj.agencyId === toAgencyId
    );

    if (fromAgency && toAgency) {
      let carToMove, brandToMove;
      fromAgency.cars.forEach((carObj, carIndex) => {
        const modelIndex = carObj.models.findIndex(
          (model) => model.carNumber === carId
        );
        if (modelIndex !== -1) {
          carToMove = fromAgency.cars[carIndex].models[modelIndex];
          brandToMove = fromAgency.cars[carIndex].brand;
          fromAgency.cars[carIndex].models.splice(modelIndex, 1);
        }
      });
      toAgency.cars.push({
        brand: brandToMove,
  ...
  getAllCars() {
    ...
    this.agencies.forEach((agency) => {
      agency.cars.forEach((car) => {
        car.models.forEach((model) => allCars.push(model));
      });
    });
    return allCars;
  },
...
const CarPurchaseManager = {
  ...
  sellCar(carId, customerId) {
    let hasMoney = null; //not sure
    let carToPurchase = null;
    let agencyWithCar = null;
    ...
    //customer
    const carPrice = carToPurchase.price;
    const buyer = this.customers.find((customer) => customer.id === customerId);
    hasMoney = buyer.cash >= 1.17 * carPrice;
